...
class DivDisLoss(nn.Module):
    ...
    def forward(self, logits):
        ...
        if mode == "mi":  # This was used in the paper
            marginal_p = probs.mean(dim=0)  # H, D     ######################mean trước rồi join 
            marginal_p = torch.einsum(
                "hd,ge->hgde", marginal_p, marginal_p
            )  # H, H, D, D
            marginal_p = rearrange(marginal_p, "h g d e -> (h g) (d e)")  # H^2, D^2

            joint_p = torch.einsum("bhd,bge->bhgde", probs, probs).mean(  ######################join trước rồi mean 
                dim=0
            )  # H, H, D, D
            ...
            joint_p=joint_p+1e-12#####################################thêm tránh nan
            marginal_p=marginal_p+1e-12#####################################thêm tránh nan 
            kl_computed = joint_p * (joint_p.log() - marginal_p.log())
            kl_computed = kl_computed.sum(dim=-1)
            kl_grid = rearrange(kl_computed, "(h g) -> h g", h=heads)############################# core mutual info
...
# This implementation is based on the PyTorch Paszke et al. (2019) and einops Rogozhnikov (2022)
# libraries. It demonstrates that the mutual information objective (3) is easily parallelized across the
# input batch using standard tensor operations.